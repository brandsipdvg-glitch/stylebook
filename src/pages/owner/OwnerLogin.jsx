...
export default function OwnerLogin() {
  ...
  const [email, setEmail] = useState('salon-1@owner.styledemo.app')
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      navigate(u.role === 'owner' ? '/salon' : '/salon', { replace: true })
    } catch (err) {
  ...
  return (
    <AuthShell
      ...
      subtitle="Manage bookings, slots and your salon profile"
      demo={
        <div className="relative z-10 mt-5 w-full max-w-sm">
          ...
          <div className="mt-3 grid grid-cols-1 gap-2">
            <button onClick={() => demoLogin('owner').then(() => navigate('/salon', { replace: true }))} className="rounded-2xl bg-brand-600 py-3 text-xs font-bold text-white shadow-glow transition hover:-translate-y-0.5">
              <FaStore className="mx-auto mb-1 h-4 w-4" />
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <button type="submit" disabled={loading} className="btn-primary w-full !py-4">{loading ? 'Signing in…' : 'Sign in as owner'}</button>
      </form>
    </AuthShell>
  )
}